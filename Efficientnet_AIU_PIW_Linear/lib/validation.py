...
def validater( valid_data_loader, model, DEVICE, val_loss_hist, val_loss_list, val_itr ):
    print( "Validating for loss" )
    ...
    with tqdm.tqdm( total=len( valid_data_loader.dataset ), desc = "Validation Progress", position=0, leave=True ) as pbar:
        for data in valid_data_loader:
            ...
            all_preds_list = [ { } for i in range( len( images ) ) ]
            pred_keys = []
            ...
            # stats_averager["validation"].send( loss_value )
        
            # update the loss value beside the progress bar for each iteration
            pbar.set_description(desc=f"Loss: {loss_value:.4f}")            
